...
export default {
  setup() {
    ...
    const practiceId = router.params.id
    const editedPractice = reactive({})
    ...
    const fetchData = async () => {
      ...
      if (error) {
        ...
        editedPractice.name = data.name
        editedPractice.email = data.email
        editedPractice.message = data.message
      }
    }
    ...
    const updatePractice = async () => {
      ...
      if (error) {
        ...
        router.push('/list') // 更新が完了したら一覧ページに戻る
      }
    }

    fetchData() // コンポーネントがマウントされたらデータを取得
